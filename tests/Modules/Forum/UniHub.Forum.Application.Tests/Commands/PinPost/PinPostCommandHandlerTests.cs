...
namespace UniHub.Forum.Application.Tests.Commands.PinPost;

public sealed class PinPostCommandHandlerTests
{
    private readonly IPostRepository _postRepository;
    private readonly PinPostCommandHandler _handler;

    public PinPostCommandHandlerTests()
    {
        _postRepository = Substitute.For<IPostRepository>();
        _handler = new PinPostCommandHandler(_postRepository);
    }

    [Fact]
    public async Task Handle_WithValidCommand_ShouldPinPost()
    {
        // Arrange
        ...
        var command = new PinPostCommand(postId, authorId);
    ...
    {
        // Arrange
        var command = new PinPostCommand(Guid.NewGuid(), Guid.NewGuid());
    ...
    public async Task Handle_WithUnauthorizedUser_ShouldReturnFailure()
    {
        ...
        var command = new PinPostCommand(postId, unauthorizedUserId);
    ...
    [Fact]
    public async Task Handle_WithAlreadyPinnedPost_ShouldReturnFailure()
    {
        // Arrange
        var authorId = Guid.NewGuid();
        var postId = Guid.NewGuid();
        var post = CreatePost(postId, authorId);
        post.Pin(); // Already pinned
