...
internal sealed class RemoveScopedPermissionCommandHandler : ICommandHandler<RemoveScopedPermissionCommand>
{
    private readonly IRoleRepository _roleRepository;
    private readonly IPermissionRepository _permissionRepository;
    private readonly IPermissionCache _permissionCache;

    public RemoveScopedPermissionCommandHandler(
    ...
    public async Task<Result> Handle(RemoveScopedPermissionCommand request, CancellationToken cancellationToken)
    {
        // Get the role
        ...
        if (!Enum.TryParse<PermissionScopeType>(request.ScopeType, ignoreCase: true, out var scopeType))
        {
            return Result.Failure(new Error(
                "RemoveScopedPermission.InvalidScopeType",
